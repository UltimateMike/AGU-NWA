...
def UserInput(): 
    ...
    # while loop runs until the user gets the PIN right or runs out of attempts
    while attempts > 0:
        try:
            ...
            if UserPinInput == UserPin:
                ...
                BankOperations()
                break  # Exit the authentication loop on success
            else:
                attempts -= 1
                ...
                if attempts > 0:
                    ...
                    print("Too many incorrect attempts. Your account is temporarily locked.")
                    sys.exit()
        except ValueError:
...
def Deposit():
    global Balance  # Update global Balance
...
def AnotherTransaction():
    # while loop ensures valid selection for continuing or exiting
    while True:
        try:
